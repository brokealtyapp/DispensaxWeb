...
- **Cross Reconciliation & Billing Module (Tarea #102):** Importa transacciones Nayax a la tabla `nayax_transactions` (categorizadas en `cash`/`card`/`other` por heurística sobre `PaymentMethod`) vía `POST /api/nayax/sync-sales` (admin/supervisor) que itera máquinas vinculadas y hace bulk upsert con conflicto en (`tenantId`, `transactionId`).
  ...
  - **Reporte exportable CSV** (`GET /api/reconciliation/cross/:id/export`): genera CSV con BOM UTF-8 incluyendo resumen financiero, discrepancia de unidades y desglose por denominación.
  ...
  - UI: tab "Facturación" en `/nayax` con selector de periodo, tarjetas (Total/Efectivo/Tarjeta/Máquinas activas) y tabla por máquina + botón Sincronizar. Panel "Cuadre Cruzado por Recolección" dentro del tab Reconciliación de `/dinero-productos` con selector de recolección, tarjetas de cuadre, alertas y botón "Exportar CSV". Permisos: `machine_sales:view` para consultas; `admin/supervisor` para sync.
