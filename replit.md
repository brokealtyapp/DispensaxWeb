...
- **Establishments CRM Pipeline:** Manages establishments through a sales pipeline (Prospecto → En Evaluación → En Negociación → Aprobado → Convertido) with follow-ups, document management, and conversion to active locations. Includes contract management (CRUD + renewal) for active establishments. El panel de detalle ofrece botones "Editar" en las tarjetas Información de Contacto y Datos Comerciales que abren modales **enfocados** ("Editar Información de Contacto" / "Editar Datos Comerciales") con sólo los campos de su tarjeta — el modal completo "Editar Establecimiento" sigue accesible desde el botón Editar del listado. La selección de sección se controla desde el padre con un estado `editingSection: 'all' | 'contact' | 'commercial'` y se pasa a `EstablishmentFormFields` mediante una prop `section` que filtra los bloques renderizados; el título y descripción del SimpleModal son dinámicos según la sección. `openEditDialog(est, section?)` precarga el form completo y setea la sección antes de abrir el modal, así el PATCH parcial omite campos no modificados (junto con el fix de #109 que hace `delete payload.x` para opcionales nullable) **además del botón "Eliminar" en el header (solo si `canDelete`) con confirmación AlertDialog** que dispara `DELETE /api/establishments/:id` y cierra el detalle al éxito. Los SimpleModal de editar establecimiento se renderizan **dentro de cada rama** del render condicional del padre (detalle vs. listado) — antes vivían solo después de un early return, lo que hacía invisible el modal cuando se abría desde el detalle. Tras `updateMutation` exitosa, la query `selectedFresh` re-fetchea y el `useEffect` mergea cambios al `selectedEstablishment` (comparando por `updatedAt`) para reflejar la edición en vivo sin cerrar el panel. Los seguimientos soportan CRUD completo: `POST/PATCH/DELETE /api/establishments/:id/followups[/:followupId]` con permisos `establishments.create/edit/delete`. Validación tenant + ownership server-side en update/delete (404 si el follow-up no pertenece al establecimiento o tenant del solicitante). PATCH usa `insertEstablishmentFollowupSchema.partial().omit({ tenantId, establishmentId, userId })` para prevenir mass-assignment. UI con botones Editar (lápiz) y Eliminar (papelera) por seguimiento — el formulario inline se reusa para crear y editar; las eliminaciones (de seguimiento o de establecimiento) usan `AlertDialog` de shadcn para confirmar.
