...
- **Layout & Tray Audit Module (Tarea #96):** Each machine declares a configurable layout of trays × lanes (`trayCount`, `lanesPerTray` defaults 6×8). Inventory items can be assigned to a specific position (`trayNumber`/`laneNumber`) by admin/supervisor. During an active service the supplier records: (a) lane changes (movement of a SKU between positions, with optional new SKU) — persisted in `lane_change_events` with `syncStatus` (pending/synced/failed/skipped) for later push to Nayax via `enqueueLaneChangeForNayax` (currently a no-op stub in `server/nayax.ts`), and (b) per-tray audits (empty positions + notes) persisted in `tray_audits`. The Money & Products back-office page exposes a "Cambios Nayax" tab listing all pending lane-change events grouped by machine, so operations can review and reconcile before sync.
