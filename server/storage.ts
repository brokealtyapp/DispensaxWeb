...
export interface IStorage {
  ...
  getEstablishmentStats(tenantId: string): Promise<{ total: number; byStage: Record<string, number>; byPriority: Record<string, number>; converted: number; activeWithContract: number; inPipeline: number; newThisWeek: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getEstablishmentStats(tenantId: string): Promise<{ total: number; byStage: Record<string, number>; byPriority: Record<string, number>; converted: number; activeWithContract: number; inPipeline: number; newThisWeek: number }> {
    const now = new Date();
    ...
    return { total: allEstablishments.length, byStage, byPriority, converted, activeWithContract, inPipeline, newThisWeek };
  }
